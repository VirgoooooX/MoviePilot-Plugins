...
class EmbyActorEnhanceLocal(_PluginBase):
    # 插件名称
    plugin_name = "Emby角色同步增强（本地版）"
    # 插件描述
    plugin_desc = "本地增强版：同步豆瓣中文演员和角色到电视剧、季及每集，并按配置刷新单集元数据。"
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/xiaoQQya/MoviePilot-Plugins/refs/heads/main/icons/actor.png"
    # 插件版本
    plugin_version = "1.1.2-local"
    # 插件作者
    plugin_author = "xiaoQQya, local"
    # 作者主页
    author_url = "https://github.com/xiaoQQya"
    # 插件配置项ID前缀
    plugin_config_prefix = "embyactorenhancelocal_"
    # 加载顺序
    plugin_order = 100
    # 可使用的用户级别
    auth_level = 1
    ...
    def _handle_media_impl(self, mediaserver: ServiceInfo, media: dict):
        ...
        # 逐集同步只按 EpisodeId 去重，不再使用季度级完成标记
                        ...
                        'content': [
                            {
                                ...
                                'props': {'cols': 12, 'md': 4},
                                'content': [
                                    {'component': 'VSwitch', 'props': {'model': 'sync_episodes', 'label': '同步每集演职人员'}}
                                ]
                            },
                            {
                                'component': 'VCol',
                                'props': {'cols': 12, 'md': 4},
                                'content': [
                                    {'component': 'VSwitch', 'props': {'model': 'refresh_episodes', 'label': '同步后刷新单集'}}
                                ]
                            },
                            {
                                'component': 'VCol',
                                'props': {'cols': 12, 'md': 4},
                                'content': [
                                    {'component': 'VSwitch', 'props': {'model': 'overwrite_episode_people', 'label': '覆盖单集已有演职人员'}}
                                ]
                            }
                        ]
                    },
