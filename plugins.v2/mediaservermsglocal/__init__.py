...
class MediaServerMsgLocal(_PluginBase):
    ...
    def send(self, event: Event):
        ...
        try:
            ...
            # 通用去重：剧集必须按 Emby 原始条目 ID/季集去重，不能使用 SeriesId，
            # 否则同一剧在短时间入库的不同集会被误判为重复。
            ...
            episode_identity = raw_item_id or "-".join(filter(None, [
                str(item_id or ""),
                str(getattr(event_info, 'season_id', '') or ''),
                str(getattr(event_info, 'episode_id', '') or ''),
                str(getattr(event_info, 'item_path', '') or '')
            ]))
            if episode_identity:
                dedupe_key = f"{server_name or ''}-{event_action_type}-{episode_identity}"
                if dedupe_key in self.__get_elements():
                    logger.info(f"忽略重复Webhook入库事件: {dedupe_key}")
                    return
                self.__add_element(dedupe_key, duration=self.DEDUPE_EXPIRATION_TIME)

            logger.info(
                "媒体服务器事件摘要: server=%s event=%s type=%s raw_type=%s "
                "series_id=%s series_name=%s item_id=%s season=%s episode=%s",
                server_name, event_type, getattr(event_info, 'item_type', None),
                raw_item.get("Type"), raw_item.get("SeriesId"),
                raw_item.get("SeriesName"), raw_item.get("Id"),
                getattr(event_info, 'season_id', None),
                getattr(event_info, 'episode_id', None)
            )
            ...
            def should_aggregate_tv() -> bool:
                ...
                # Emby 解析器通常会保留完整 json_object；即使上游只传递了
                # 标准化字段，也允许使用 item_id/剧名/路径作为聚合键。
                return bool(
                    getattr(event_info, 'item_id', None)
                    or getattr(event_info, 'item_name', None)
                    or getattr(event_info, 'item_path', None)
                )
    ...
    def _get_series_id(self, event_info: WebhookEventInfo) -> Optional[str]:
        ...
        try:
            ...
            # Emby 解析器会把 SeriesId 写入 item_id。
            item_id = getattr(event_info, "item_id", None)
            if item_id:
                return str(item_id)

            # 最后使用剧名或路径中的剧集目录作为稳定兜底键。
            item_name = getattr(event_info, "item_name", None)
            if item_name:
                return str(item_name).split(" S", 1)[0].strip()
            item_path = getattr(event_info, "item_path", None)
            if item_path:
                normalized = str(item_path).replace("\\", "/")
                match = re.search(r"/([^/]+?)(?:/Season\s*\d+)?/[^/]+$", normalized, re.I)
                if match:
    ...
    def _aggregate_tv_episodes(self, series_id: str, event_info: WebhookEventInfo):
        ...
        try:
            ...
            # 静默窗口聚合：同一电视剧每有新一集入库，都取消旧定时器并
            # 重新等待完整聚合时间。只有连续指定秒数没有新事件后才发送一次，
            # 确保整批剧集完成入库后按电视剧汇总通知。
            if series_id in self._aggregate_timers:
                try:
                    self._aggregate_timers[series_id].cancel()
                except Exception as e:
                    logger.debug(f"取消旧聚合定时器时出错: {str(e)}")

            logger.info(f"刷新剧集聚合静默窗口: series_id={series_id}, {self._aggregate_time}秒")
            try:
    ...
    def stop_service(self):
        ...
        """
        try:
            # 停用、重载或系统退出时不得提前发送聚合队列，否则每次插件
            # 重载都会制造一条“短时间重复通知”。等待中的事件直接丢弃。
            if self._pending_messages:
                logger.info(f"插件停止，取消 {len(self._pending_messages)} 个待发送剧集聚合队列")
